...
export default function () {
  ...
  const userPlayerIndex = players.findIndex(({ id }) => id === userData.id)
  const userAlreadyPlay = Boolean(userPlayerIndex === 0 ? firstPlayerPlay : secondPlayerPlay)
  const buttonCommonProps = {
    isDisabled: userAlreadyPlay,
    variant: 'ghost',
    boxSize: '32',
    fontSize: '6xl',
    colorScheme: 'blue'
  ...
  const playerIsInGame = players.some(({ id }) => id === userData.id)
  const isRoomInGame = players.length < 2
    ...
    <>
      <Flex justifyContent='center' gap='7' my='4'>
        {
          playerIsInGame
            ...
            : isRoomInGame && <Button onClick={joinGame} w='135px' colorScheme='green'>Join game</Button>
        }
      ...
      <Flex justifyContent='center' gap='7' wrap='wrap' my='7'>
        <Square color={firstPlayerRateColor} {...PLAYERS_SQUARE_STYLES}>
          {players[0] && <Text>{players[0].nickName} {userPlayerIndex === 0 && '(You)'}</Text>}
        ...
        <Square color={secondPlayerRateColor} {...PLAYERS_SQUARE_STYLES}>
          {players[1] && <Text>{players[1].nickName} {userPlayerIndex === 1 && '(You)'}</Text>}
